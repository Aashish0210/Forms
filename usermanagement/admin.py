...
class DailyReportInline(admin.TabularInline):
    model = DailyReport
    extra = 0
    fields = ['date', 'start_time', 'end_time','total_hours', 'task_done', 'problem_faced']
...
class EmployeeDailyActivityInline(admin.TabularInline):
...
# Inline for Supervisor Next Day Planning
class EmployeeNextDayPlanningInline(admin.TabularInline):
    model = EmployeeNextDayPlanning
    extra = 0
    fields = ['date', 'coordination', 'start_time', 'end_time', 'total_hours', 'to_do']
    can_delete = True  # Allow deletion of individual supervisor planning records
# Allow deletion of individual supervisor planning records
...
class ProjectManagementFormInline(admin.TabularInline):
    ...
    def clean_supervisor(self):
        ...
        return supervisor  # Allow deletion of individual project management forms
...
class EmployeeProfileAdmin(admin.ModelAdmin):
    ...
    inlines = [EmployeeDailyActivityInline, EmployeeNextDayPlanningInline]
...
admin.site.register(EmployesProfile, EmployeeProfileAdmin)
